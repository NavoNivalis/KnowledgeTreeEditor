...
export async function deleteTree(treeId: string): Promise<void> {
  ...
  try {
    // 1. 先查出该树下所有节点的 ID
    const { data: nodes, error: fetchNodesError } = await createClientSupabase()
      .from('tree_nodes')
      .select('id')
      .eq('tree_id', treeId);
    ...
    if (nodeIds.length > 0) {
      const { error: noteError } = await createClientSupabase()
        .from('notes')
        .delete()
        .in('node_id', nodeIds);
    ...
    // 3. 删除该树下所有节点
    const { error: nodeError } = await createClientSupabase()
      .from('tree_nodes')
      .delete()
      .eq('tree_id', treeId);
    ...
    // 4. 删除树本身
    const { error: treeError } = await createClientSupabase()
      .from('trees')
      .delete()
      .eq('id', treeId);
